...
また、WebUIの設定で、画面に表示する顔を「KariPom」「Miss KariPom」「Face Gallery」の3つのFace Modeから切り替えられます（第10章）。KariPomとMiss KariPomは見た目や機能に大きな違いはなく、気分に合わせて選べる小さなお楽しみです。Face Galleryを選ぶと、お気に入りの画像を固定表示したり、顔を出さずにLighting・Visualizerの演出だけを画面いっぱいに楽しんだりできます。
...
| 表情表示（Face Mode） | 本体の画面に表示する顔を「KariPom」「Miss KariPom」「Face Gallery」の3モードから選べます。Face GalleryではPNG画像のほか「None（顔を表示しない）」も選べ、顔なしでLighting・Visualizerだけを表示できます。 | 第10章 |
| 睡眠・起床モード | しばらく操作がないと自動的に眠り、眺めて楽しいSleep表示（Sleep Lighting Carousel）を行います。触れる・話しかける・動かすと目を覚まします。Face Modeが「Face Gallery」のときは眠りません。 | 第10章 |
...
| Visualizer（音の可視化） | 音の強さやリズムを、画面いっぱいのグラフや図形として表示します。演出は8種類で、WebUIでは「OFF」を含む9項目から選べます。 | 第13章 |
...
- **画面の演出：Visualizer と Lighting** — 音を可視化する層（Visualizer）と、背景の光の演出（Lighting）を重ねて表示できます。顔はどちらの演出よりも常に手前（最前面）に表示されるよう作られているため、演出が派手なときでもかりポムの表情は隠れません。逆に、顔を出さずに演出だけを画面いっぱいに楽しみたいときは、Face Modeで「Face Gallery」＋「None」を選べます（第10章）。
...
| **Face Gallery** | 表情PNGの一覧と切り替え。「None（顔を表示しない）」もここで選びます。 | 第10章 |
...
## 10.1 Face Mode（顔の3つのモード）

かりポムの画面に何を表示するかは、WebUIのHome画面にある**Face Mode**で切り替えます。次の3つから選べます。

| Face Mode | 画面に表示されるもの | Lighting・Visualizer | 自動Sleep |
|---|---|---|---|
| 🐰 **KariPom** | かりポムの通常の顔 | 使えます | します |
| 🎀 **Miss KariPom** | まつ毛付きの「Miss かりポム」の顔 | 使えます | します |
| 🖼 **Face Gallery** | 選んだPNG画像、または顔なし | Noneを選んだときだけ使えます | しません |

「KariPom」と「Miss KariPom」の違いは、目のまつ毛の有無だけです。どちらを選んでも、瞬き・鼻のヒクヒク・口パクなどの動きや、Lighting・Visualizerの使い方は同じです。

出荷時（設定を保存していない状態）は「KariPom」から始まります。

**Face Galleryモードの2つの表示**

Face Modeで「Face Gallery」を選ぶと、**Face Galleryページで選んだ内容がそのまま表示され続けます**。Face Galleryページには、SDカードの `faces/` フォルダに入れた表情PNGの一覧と、**None（顔を表示しない）**が並びます。

| Face Galleryでの選択 | 画面の表示 |
|---|---|
| PNG画像 | 選んだ320×240の画像を画面いっぱいに表示し続けます。この間、Lighting・Visualizerは表示されません。 |
| **None** | 顔を一切表示しません。かわりに、選んでいるLighting（第14章）・Visualizer（第13章）が画面いっぱいに表示され、そのまま動き続けます。 |

💡 **顔を出さずに演出だけを眺めたいときは、Face Mode →「🖼 Face Gallery」、Face Galleryページ →「🚫 None」**を選んでください。音に反応する画面演出を、インテリア照明のように楽しむ使い方ができます。

**操作の手順**

1. WebUIのHome画面で、Face Modeの**🖼 Face Gallery**を選びます。
2. 同じセクションの「🎨 顔を選ぶ / None」ボタン（またはFile Managerの「🎨 顔を選ぶ」）からFace Galleryページを開きます。
3. 表示したいPNGの**🎭 Wear**ボタン、または顔を出したくない場合は**🚫 None**ボタンを押します。

いま選ばれている項目には緑色の枠が付きます。Face Galleryページのボタンを押すと、Face Modeも自動的に「Face Gallery」へ切り替わります。

通常の顔に戻したいときは、Home画面で「🐰 KariPom」または「🎀 Miss KariPom」を選びます（File Managerの「🙂 標準の顔に戻す」でも同じように戻せます）。このときFace Galleryで選んでいたPNGは記憶されたままなので、あとで「Face Gallery」を選び直せば同じ画像が再び表示されます。

【図：Face Galleryページのスクリーンショット】

【図：表情一覧（各表情の画面写真を並べたもの）】

💡 Face ModeとFace Galleryの選択は、どちらも本体に保存されます。電源を入れ直しても、前回選んでいた状態から始まります。
...
Face Modeが「Face Gallery」＋「None」のときは、意図的に顔（目・まつ毛・鼻・口）を表示しないため、口パクの動きも画面には現れません。音への反応はLighting・Visualizerの演出として楽しめます。

💡 表情は今後のファームウェア更新で追加されることがあります。最新の表情はFace Galleryページでご確認ください。ご自身で用意した320×240のPNGを追加することもできます（第17章）。
...
Face Modeが「KariPom」または「Miss KariPom」のとき、約15分間、操作（WebUI操作・音・タッチ・動きなど）がないと、あくびをしてから眠りに入ります。

⚠️ **Face Modeが「Face Gallery」のときは、PNG・Noneのどちらを選んでいても自動的に眠りません。**Face Galleryは「選んだ表示をそのまま保ち続けるモード」だからです。選んだPNGが途中で時計や別の画像に変わることはなく、Noneのときも顔なしのままLighting・Visualizerが動き続けます。眠らせたいときは、Face Modeを「KariPom」または「Miss KariPom」に戻してください（戻した時点から15分の計測が始まります）。
...
## 13.2 選べるVisualizer（OFFを含む9項目／演出は8種類）

WebUIのVisualizer選択には、演出を出さない「OFF」を含めて9項目が並びます。このうち実際の演出は次の8種類です。

| 名前 | どんな見た目か |
|---|---|
| OFF | 何も表示しません。通常の顔のままです（演出8種類には含みません）。 |
...
【図：Visualizer各モードの画面写真（OFFを含む9枚）】

💡 どのVisualizerを選んでも、かりポムの顔は常に一番手前（最前面）に表示されるように作られています。Visualizerの演出で顔が完全に隠れてしまうことはありません。なお、Face Modeで「Face Gallery」＋「None」を選んだときだけは、顔を表示せずVisualizerだけを画面いっぱいに表示できます（第10章）。
...
- 顔は、LightingやVisualizerよりも常に手前（最前面）に表示されます（Face Modeで「Face Gallery」＋「None」を選んだときは、顔を表示せずLightingだけを楽しめます。第10章）。
...
| Aquarium | 熱帯魚が泳ぐ水槽をイメージしたLighting。魚、水草、気泡、岩、砂地などを描画する、眺めて楽しむタイプのモードです。 |
| Flying Pompadour | 往年のスクリーンセーバー文化へのオマージュとして、羽の生えたKariPom/CoreS3風キャラクターが右上から左下へ飛ぶLightingです。 |
| Rainbow Washing Machine | 極彩色の色片が中心から外へ広がり、複数の逆回転層が渦を作るトランス風Lightingです。 |
| Pixel Invasion | 黒背景にカラーのピクセル宇宙人編隊、自機、防弾シールド、弾、時折横切るUFOを描くレトロアーケード風Lightingです。 |
| Flower Clock | 画面いっぱいに広がる12色の放射状の区画が虹色の花時計になり、中央の白い文字盤に長針・短針でその時々の時刻を表示するLightingです。数字は12・3・6・9のみ表示し、各時刻の方向がそれぞれの色の中心になります。 |
| PINBALL Arcade | かりポムの顔がそのままピンボール台になる、完全自動プレイのスクリーンセーバーです。目が丸型バンパー、下部のフリッパーが自動でボールを打ち返し続けます。 |
| BASEBALL Arcade | ATARI2600〜初期ファミコン時代を思わせる、完全自動プレイの野球デモです。投球・打撃・走塁のサイクルが無限に続きます。 |
| SKY BURNER | 1980年代後半のアーケード空中戦ゲームを思わせる、自機後方視点の疑似3D空中戦デモです。索敵→追尾→ロックオン→ミサイル発射→命中→爆発という流れを自動的に繰り返し、時折敵機からもミサイルが飛んできてすれ違います。 |
...
画面下には**Character／Visualizer／Lighting**の3つのボタンがあり、それぞれ左1/3クリックで前へ、右1/3クリックで次へ切り替わります（中央1/3は無操作）。**KariPom Desktop**と同じ、KariPom／Miss KariPom／None（顔を表示しない）のCharacter、全9種類のオーディオビジュアライザー、全25種類のLighting（背景演出）を、CoreS3実機がなくてもCompanion単体で確認できます。PC音声（Talk）に反応するVisualizer（第13章）の演出もあわせて試せます。
...
- `faces/` — Face Gallery（第10章）やランダムFaceなどで使う表情PNGを入れるフォルダです。320×240のPNGを追加すると、Face Galleryページの一覧に表示されます。
...
| 項目 | 説明 |
|---|---|
| Visualizer選択 | OFFを含む9項目（演出は8種類）から1つ選択。 |
...
## 19.4 かりポムの名前とFace Mode

| 項目 | 説明 |
|---|---|
| かりポムの名前 | 最大32文字までの好きな名前を設定できます。設定すると、IPアドレスの代わりに名前が表示されます（第8章）。空欄にするとIPアドレス表示に戻ります。 |
| Face Mode | 画面に表示する顔を「KariPom」「Miss KariPom」「Face Gallery」の3種類から選べます。Face Galleryを選んだ場合は、Face GalleryページでPNGまたはNone（顔を表示しない）を選びます（第10章）。 |
...
- 音源が内蔵マイク（MIC）になっていないか確認してください。内蔵マイクモードでVisualizerを選ぼうとすると「内蔵マイクモードではAudio Visualizerは使用できません。」という案内が表示され、選択はOFFへ戻ります（第12章・第13章）。
- 音源をPC音声（Wi-Fi）にしている場合は、KariPom Companionが起動してFFTデータを送信しているか確認してください。届いていない場合は案内が表示され、選択はOFFへ戻ります。
...
**Q. 筐体の色は決まっていますか？**
A. いいえ、自由に選べます。筐体は3Dプリントで製作するため、お好きな色・フィラメントで作れます。本書に登場するピンクとティファニーブルーは、作者が製作した2台の例です。機能に色による違いはありません。Face Modeの「KariPom」「Miss KariPom」の違い（まつ毛の有無）も、機能的な差はありません。
...
| Ver. 4.11 | 2026年8月 | 顔まわりの設定を「Face Mode」として整理したことに合わせて更新。第10.1章を「Face Gallery（表情ギャラリー）」から「Face Mode（顔の3つのモード）」へ書き換え、KariPom／Miss KariPom／Face Galleryの3モードと、Face GalleryでのPNG／None（顔を表示しない）の選び方・操作手順を追記。Face Gallery＋Noneでは顔を表示せずLighting・Visualizerを画面いっぱいに表示できること、Face Modeが「Face Gallery」のときはPNG・Noneのどちらでも自動Sleepしないことを第10.3章（睡眠モード）に明記。第3章の機能一覧、第9章のWebUIページ一覧、第13章・第14章の「顔は最前面」の補足、第17章の`faces/`フォルダ説明、19.4を新仕様へ更新。用語集の「キャラクタースタイル」を「Face Mode」「Face Gallery」「None（顔を表示しない）」の3項目へ差し替えた。 |
...
| Visualizer | ビジュアライザー。音の大きさやリズムを画面いっぱいの図形として表示する機能。演出は8種類で、WebUIの選択肢は「OFF」を含めて9項目。 |
...
| Face Mode | フェイスモード。画面に表示する顔を「KariPom」「Miss KariPom」「Face Gallery」の3種類から選ぶ設定（第10章）。以前の「キャラクタースタイル」（かりポム／Miss かりポムの2択）を、Face Galleryを含めて整理したもの。 |
| Face Gallery | フェイスギャラリー。SDカードの表情PNGを選んで表示する機能・ページ名。「None（顔を表示しない）」も選べる。 |
| None（顔を表示しない） | Face Galleryの選択肢のひとつ。顔を表示せず、Lighting・Visualizerだけを画面いっぱいに表示する（第10章）。 |
