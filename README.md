...
本プロジェクトは、個人製作作品の記録・共有を目的として公開しています。商品化・販売・工作キット化を目的としたものではありません。製作にはArduino、電子工作、3Dプリント等の基礎知識が必要です。作者による個別の製作サポートは行っていませんが、公開している2冊のマニュアルとソースコードは、AI（ChatGPT・Claudeなど）に読み込ませて相談しながら進める使い方を想定して書かれています。
...
なお、LINE OUT増設とBluetooth入力（MH-M18）は設計・手順書まで完成していますが、作者の実機には未実装のオプションです。
