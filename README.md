...
表情表示と音声再生、サーボによる首の上下・左右動作、PC音声とのリアルタイム口パク連動（BBX）、Visualizer（8種）・Lighting（20種）による画面演出、Webブラウザから操作できるWeb UI（KariPom Lab）、ジョイスティック操作、独り言機能、睡眠・起床モード、SDカードによるログ・音声ファイル管理など。拡張ハードウェア「Karipom Ear」（PCM1808＋Raspberry Pi Pico 2 H）を追加すると、PCなしでライン入力の音に反応するBBXを楽しめます。
